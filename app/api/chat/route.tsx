...
async function readFiles() {
  try {
    ...
    const summary = fs.existsSync(path.join(meDir, "summary.txt"))
      ? fs.readFileSync(path.join(meDir, "summary.txt"), "utf-8")
      : "";

    const resumeText = fs.existsSync(path.join(meDir, "Tejal_resume.txt"))
      ? fs.readFileSync(path.join(meDir, "Tejal_resume.txt"), "utf-8")
      : "";

    const linkedinText = fs.existsSync(path.join(meDir, "Tejal_Linkedin.txt"))
      ? fs.readFileSync(path.join(meDir, "Tejal_Linkedin.txt"), "utf-8")
      : "";

    console.log("✅ Summary:", summary.length, "chars");
    console.log("✅ Resume:", resumeText.length, "chars");
    console.log("✅ LinkedIn:", linkedinText.length, "chars");
    ...
    while (!done) {
      ...
      if (
        ...
        for (const toolCall of choice.message.tool_calls) {
          ...
          let result: { recorded: string } = { recorded: "ok" };
